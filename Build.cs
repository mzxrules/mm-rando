...
namespace MMRando
{
    ...
    public partial class MainRandomizerForm
    {
        ...
        private void MakeRom(string inputFilename, string outputFilename, BackgroundWorker worker)
        {
            using (BinaryReader baseRom = new BinaryReader(File.Open(inputFilename, FileMode.Open, FileAccess.Read)))
            {
                ROMFuncs.ReadFileTable(baseRom);
            }
            ...
            byte[] ROM = ROMFuncs.BuildROM(outputFilename);
            if (_outputVC)
            {
                worker.ReportProgress(98, "Building VC...");
                ROMFuncs.BuildVC(ROM, VCDirectory, Path.ChangeExtension(outputFilename, "wad"));
            }
